#!/usr/bin/env python3
"""
~/bin/ renameYtWithPrefixSeqNumbersBasedOnPlaylistOrder.py
This script renames YouTube videos prefixing them with the
  number (zfill'ed) that is the same as its ordered position
  in playlist.

For this script to work, there must exist a file
  in folder named youtube-ids
...
import lblib.ytfunctions.yt_str_fs_vids_sufix_lang_map_etc as ytfs  # .read_ytids_from_strlines

# this lambda is intended to be used in a 'filter',
# each element is filtered-in if lambda returns True
# for it (filterOutNon11Char will be used this later below)


def extract_ids(ytids_filename):
  lines = open(ytids_filename).readlines()
  ytids = ytfs.read_ytids_from_strlines(lines)
  return ytids
...
class Rename:
  ...
  def process_rename(self):
    ...
    if self.confirm_before_rename:
      bool_confirm = self.confirm_renames()
  ...
  def find_renames(self):
    ...
    # sorted( filenames )
    usedfilenames = []
    n_to_align_left_zeroes = len(
      str(len(self.ytids)))  # another option is to encapsulate a function that uses math.log10()
    for filename in filenames:
      videoid = get_videoid_from_filename(filename)
      if videoid not in self.ytids:
      ...
      try:
        ...
        raise ValueError
      seqnumber = index + 1
      new_name = str(seqnumber).zfill(n_to_align_left_zeroes) + ' ' + filename
  ...
  def show_renames(self):
    ...
    for i, rename_tuple in enumerate(self.rename_pairs):
      seq_number = i + 1
      current_name, new_name = rename_tuple
      print('Rename n.', seq_number)
      print(' => ', current_name)
      print(' => ', new_name)

  def confirm_renames(self):
    if len(self.rename_pairs) > 0:
      msg_for_input = 'Confirm renames above with %d files and %d ids? (Y*/n) ' % (len(self.rename_pairs),
                                                                                   len(self.ytids))
  ...
  def do_renames(self):
    n_of_renames = 0
    for i, rename_tuple in enumerate(self.rename_pairs):
      old_filename, new_filename = rename_tuple
      if not os.path.isfile(old_filename):
        continue
      os.rename(old_filename, new_filename)
      n_of_renames += 1

    print('Total of renames = %d' % n_of_renames)
