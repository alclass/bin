#!/usr/bin/env python3
"""
~/bin/renamePrefixingSeqNumbers.py
  Prefixes filenames with its alphanumerical-order seq-number

Usage:
  enamePrefixingSeqNumbers.py -e=[<extension>]
...
Running this script and confirming the operation, the (renamed) new names will be:
  "1 F1 file foo bar.mp4"
  "2 F2 file bar foo.mp4"
...
def return_only_filenames_under_ext(filenames, dir_abspath=None, extension=None):
  ...
  # split the abspaths getting back their filenames
    as filenames = [os.path.split(abspath)[1] for abspath in absfiles]
...
class Renamer:
  ...
  def __init__(self, ext_for_rename=None, dir_abspath=None):
    ...
    if ext_for_rename is None:
      self.extension = DEFAULT_DOT_EXTENSION
    else:
      self.extension = ext_for_rename
    self.extension = adjust_front_dot_if_needed(self.extension)
  ...
  def find_files_from_current_folder(self):
    ...
    filenames = return_only_filenames_under_ext(files, self.dir_abspath, self.extension)
    ...
    self.rename_pairs = []
    for i, ip_filename in enumerate(filenames):
      seq = i + 1
      seqstr = str(seq).zfill(zfill_size)
      op_filename = f"{seqstr} {ip_filename}"
      rename_pair = ip_filename, op_filename
