...
~/bin/renamePrefixWithInitials.py
  ...
  2) if two words can be drawn, the prefix is composed of the first letter of first word
     and the first letter of second word, both Capitalized
...
class InitialsPrefixRenamer:
  ...
  def prep_renames(self):
    if self.dot_extension is None:
      ...
      files = glob.glob('*' + self.dot_extension)
    if len(files) == 0:
